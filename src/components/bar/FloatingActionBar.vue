...
const isSetupActive = ref(props.isSetupActive);
...
const handleClick = <T>(item: Item<T>) => {
  emits("itemClicked", item.payload);
...
const handleSetupClick = () => {
  emits("setupClicked");
  isSetupActive.value = !isSetupActive.value;
};
  ...
  <div class="flex flex-col w-96 gap-y-1 items-baseline">
    ...
    >
      ...
      >
        ...
        <span class="pl-1.5 text-gray-800 group-hover:text-blue-500">{{ item.label }}</span>
      </div>
    </div>
    ...
    >
      ...
      <div
        ...
        :class="{
          '!border-blue-600 text-blue-600 !fill-blue-600': isSetupActive,
        }"
