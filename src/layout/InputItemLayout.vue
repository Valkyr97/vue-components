...
<template>
  ...
  >
    <div class="flex gap-x-2 absolute top-1.5 right-3 h-fit w-fit items-center">
      <svg
        :class="{ 'stroke-blue-500': isSettingsActive }"
        @click="handleSetupIconClick"
      ...
      <FormKitIcon
        @click="$emit('close')"
        icon="close"
        class="flex w-3.5 z-20 cursor-pointer"
      />
    </div>
