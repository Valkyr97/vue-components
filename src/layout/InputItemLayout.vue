...
<template>
  <div
      class="w-full border-2 border-gray-300 rounded-lg bg-gray-100 pb-4 transition relative text-gray-500 overflow-hidden"
      :class="{'custom-border border-2': selected}"
  >
    <div :class="ignoreTargetClass" class="flex gap-x-2 absolute top-1.5 right-3 h-fit w-fit items-center">

      <svg :class="{'stroke-blue-500': isSettingsActive}" @click="handleSetupIconClick"
           class="w-4 h-full z-20 stroke-1 fill-gray-500 cursor-pointer"
           viewBox="0 0 20 18"
      ...
      <FormKitIcon @click="$emit('close')" icon="close" class="flex w-3.5 z-20 cursor-pointer"/>
    </div>
    <Holder class="!bg-transparent" hold-class="handler"/>
    <slot :isSettingsActive="isSettingsActive"/>
  </div>
</template>
